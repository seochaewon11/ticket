/* ============================================
   ...
   (function (global) {
    ...
    /* ---------- localStorage Key ---------- */
    var STORAGE_KEY = 'noli_app_data_v2';
    ...
    /* ---------- 설정완료 화면(complete.html)에서 사용하는 취향 해시태그 매핑 ---------- */
    var MOOD_TAG_MAP = {
      '에너지 넘치는': '#에너지충만',
      '감성적인': '#감성적인',
      '유쾌한': '#유쾌한',
    ...
    var RECENTLY_VIEWED = [
      { id: 'recent_001', title: '라 트라비아타', venue: '예술의전당', posterUrl: 'img/playimage04.png' },
      { id: 'recent_002', title: '위키드', venue: '샤롯데씨어터', posterUrl: 'img/wicked.jpg' }
    ];
    ...
    /* ---------- 상세 리포트(analysis.html)에서 사용하는 데이터 ---------- */
    var VIEWING_TREND = {
      totalCount: 12,
      months: [
        { label: '9월', count: 2 },
    ...
    var DEFAULT_DATA = {
      ...
      performances: [
        ...
        },
        {
          ...
          venue: '고척스카이돔',
          synopsis: '전 세계를 무대로 한 콜드플레이의 내한 공연. 대형 스타디움을 가득 채우는 웅장한 사운드와 화려한 조명 연출, 관객과 함께 만드는 떼창까지 — 잊지 못할 라이브 경험을 선사합니다.',
          liked: false,
          status: null
        },
        {
          ...
          venue: '예술의전당 오페라극장',
          synopsis: '크리스마스 이브, 클라라가 선물 받은 호두까기 인형이 왕자로 변신하며 펼쳐지는 환상의 여정. 눈송이 왈츠와 꽃의 왈츠 등 차이콥스키의 아름다운 선율과 함께하는 겨울 시즌 대표 발레 공연입니다.',
          liked: false,
          status: null
        },
        {
          ...
          period: '2026.10.05',
          venue: '롯데콘서트홀',
          synopsis: '국내외 필하모닉 오케스트라가 한자리에 모여 선사하는 스페셜 갈라 콘서트. 대중에게 친숙한 클래식 명곡부터 오케스트라의 웅장함을 느낄 수 있는 대편성 곡까지 다채로운 프로그램으로 구성됩니다.',
          liked: false,
          status: null
        }
      ],
    ...
    function init() {
      ...
      Object.keys(DEFAULT_DATA.user).forEach(function (key) {
        if (state.user[key] === undefined) state.user[key] = clone(DEFAULT_DATA.user[key]);
      });
      ...
      (state.performances || []).forEach(function (p) {
        var def = DEFAULT_DATA.performances.find(function (d) { return d.id === p.id; });
        if (!def) return;
        Object.keys(def).forEach(function (key) {
          if (p[key] === undefined) p[key] = clone(def[key]);
        });
      });
      persist();
      return state;
    ...
    function updatePreference(favoriteGenres, favoriteMoods) {
      ...
      data.userPreference = {
        ...
        favoriteArtistIds: data.userPreference.favoriteArtistIds || [],
    ...
    /* 저장된 취향(mood/genre)을 기반으로 설정완료 화면용 해시태그 3개 + 고정 태그 1개를 생성
    ...
    /* ---------- 상세 리포트(analysis.html) 조회 ---------- */
    function getViewingTrend() {
      return VIEWING_TREND;
    }
    ...
    function getTopArtistsRanking() {
      return TOP_ARTISTS_RANKING;
    }
    ...
    global.NoliData = {
      ...
      getViewingTrend: getViewingTrend,
      getGenreDistribution: getGenreDistribution,
      getTopArtistsRanking: getTopArtistsRanking,
